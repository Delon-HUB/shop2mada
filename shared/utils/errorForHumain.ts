...
const DIC_ERROR_MESSAGE: Record<EError, string> = {
  ...
  [EError.INCOMPLETE_DATA]:
    "Données incomplètes. Veuillez vérifier tous les champs",
  ...
  [EError.PAYMENT_REFUND_NOT_ALLOWED]:
    "Remboursement non autorisé pour ce paiement",
  [EError.ERR_NETWORK]:
...
export const translateError = (errorMessage: EError): string => {
  return (
    DIC_ERROR_MESSAGE[errorMessage] ?? "Une erreur inattendue s'est produite"
  );
};
